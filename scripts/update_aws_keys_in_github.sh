...
function update_aws_keys_in_github() {
  ...
  then
    ...
    gh secret set --env $env AWS_ACCESS_KEY_ID -b"${AWS_ACCESS_KEY_ID}"
    gh secret set --env $env AWS_SECRET_ACCESS_KEY -b"${AWS_SECRET_ACCESS_KEY}"
    gh secret set --env $env AWS_SESSION_TOKEN -b"${AWS_SESSION_TOKEN}"
  fi
}
